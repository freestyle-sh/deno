...
- fix(workers): `importScripts` concurrently and use a new `reqwest::Client` per
  importScripts (#23699)
- fix: DOMException doesn't throw on \_\_callSitesEvals (#23729)
...
- fix(cli): unbreak extension example and fix \_\_runtime_js_sources (#22906)
...
- fix(zlib): handle no flush flag in handle\_.write (#21432)
...
- fix(cli): Support using both `--watch` and `--inspect` at the same time
  (#20660)
- fix(cli): panic with \_\_runtime_js_sources (#20704)
